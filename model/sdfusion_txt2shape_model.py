...
from model.networks.diffusion_networks.samplers.ddim import DDIMSampler

# distributed 
from model.diff_utils.distributed import reduce_loss_dict

# rendering
from model.diff_utils.util_3d import init_mesh_renderer, render_sdf
...
class SDFusionText2ShapeModel(BaseModel):
    def __init__(self, opt):
        ...
        assert opt.network.df_cfg is not None
        assert opt.network.vq_cfg is not None

        # init df
        df_conf = OmegaConf.load(opt.network.df_cfg)
        vq_conf = OmegaConf.load(opt.network.vq_cfg)
        ...
        # init vqvae
        self.vqvae = load_vqvae(vq_conf, vq_ckpt=opt.network.vq_ckpt, opt=opt)
        ...
        # if opt.network.ckpt is not None:
        #     self.load_ckpt(opt.network.ckpt, load_opt=self.isTrain)
