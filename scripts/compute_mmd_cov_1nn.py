...
try:
    ...
    print("distChamferCUDA not available; fall back to slower version. You should better make this work.")
    def distChamferCUDA(x, y):
...
def sample_pc(pc, number=5000, transform=False, visual=False):
    if transform:
        ...
        pc = pc.dot(rotation_matrix.T)
    n_points = pc.shape[0]
    np.random.seed(42)
    if n_points >= number:
        random_indices = np.random.permutation(n_points)[:number]
        resampled_point_cloud = pc[random_indices]
    else:
        random_indices = np.random.randint(0, n_points, size=number)
        resampled_point_cloud = pc[random_indices]

    return resampled_point_cloud

# def normalization(pc):
#     pc -= np.mean(pc)
#     shape_size = np.max(pc, axis=0) - np.min(pc, axis=0)
#     pc_norm = pc * (2/shape_size)
#     return pc_norm
def normalization(pc, visual=False):
    pc -= np.mean(pc, axis=0)
    max_range = np.max(np.abs(pc))
    pc_norm = pc / max_range
    if visual:
        visual_pc(pc)
    return pc_norm

def collect_pcs(path, cat, number=5000, transform=False, visual=False):
    pc_list=[]
    for root, dir, files in os.walk(path):
        for file in files:
            if file.split('_')[0] != cat:
                continue
            obj = trimesh.load(os.path.join(root, file))
            pc = copy.deepcopy(obj.vertices)
            pc = sample_pc(pc, number=number, transform=transform)
            pc = normalization(pc, visual=visual)
            pc = torch.Tensor(pc)
            pc_list.append(pc)
    pcs = torch.stack(pc_list, dim=0)
    return pcs
